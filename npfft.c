...
/*
 *  Ported by Copyright (C) 2020-2021 CHEN Siyu
 *
 *  Original author: Copyright (C) 2004-2018 Max-Planck-Society
...
/*
consume n_fft data samples, output idealy n_fft//2 + 1 features.
if n_fft/2 is even, then results are [real,imag,real,imag,...] if size nfft/2
if nfft//2 is odd, then results are given by [real,real,imag,real,image,....,real]
  zero imag shall be inserted into [1] and appended to [-1]
if nfft/2 is odd, but nfft is even, then reuslts are given by [real,real,imag,real,imag]
  zero imag shall be inserted into [1].

this function accept only 2^n even input, which mean condition(2) holds.
*/
int fft(double * signal,int n_fft, double * out)
{
  int half_n_fft = n_fft/2;  
  int out_fft_dims = half_n_fft + 1; // frequencies and flat current 
  int out_fft_real_imag_total_elements = out_fft_dims * 2;
  memset(out,0,sizeof(double)*out_fft_real_imag_total_elements);

  double fct = 1; //keep the magnitude information in the spectragram
  rfft_plan plan = make_rfft_plan(n_fft); // plan should be not NULL
  int status = rfft_forward(plan,signal,fct); //status should be 0
  memcpy(out+1,signal,sizeof(double)*n_fft);
  out[0] = out[1];
  out[1] = 0;
  if (plan)
